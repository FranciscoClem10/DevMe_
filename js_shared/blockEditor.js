...
(function (global) {
  ...
  function makeInnerDrop(parentId, childType, index) {
    ...
    // Touch drop handler
    dz._touchDropHandler = function(touchDragInfo, syntheticEvent) {
      const parentInfo = findBlockDeep(parentId);
      if (!parentInfo) return;
      
      // Set dragData for handleDrop
      dragData = touchDragInfo;
  ...
  function setupEmptyDrop(el, parentId, childType, index) {
    ...
    // Touch drop handler
    el._touchDropHandler = function(touchDragInfo, syntheticEvent) {
      const parentInfo = findBlockDeep(parentId);
      if (!parentInfo) return;
  ...
  function makeElseIfDrop(parentId, index) {
    ...
    // Touch drop handler
    dz._touchDropHandler = function(touchDragInfo, syntheticEvent) {
      const type = touchDragInfo.source === 'palette' ? touchDragInfo.type : null;
      if (touchDragInfo.source === 'palette' && type !== 'sino_si') return;
  ...
  function setupRowDrop(row, parentArr, indexInParent) {
    ...
    // Touch drop handler
    row._touchDropHandler = function(touchDragInfo, syntheticEvent) {
      if (syntheticEvent.target && syntheticEvent.target.closest && syntheticEvent.target.closest('.block-inner-dz')) return;
      
      // Set dragData for handleDrop
      dragData = touchDragInfo;
      
      const rect = row.getBoundingClientRect();
      const before = syntheticEvent.clientY < rect.top + rect.height / 2;
      const idx = before ? indexInParent : indexInParent + 1;
      const rowBlockId = row.dataset.blockId;
      const rowInfo = rowBlockId ? findBlockDeep(rowBlockId) : null;
      const draggedType = incomingDragType();
  ...
  function setupWorkspaceDrop() {
    ...
    // Touch drop handlers
    ws._touchDropHandler = function(touchDragInfo, syntheticEvent) {
      if (syntheticEvent.target && syntheticEvent.target.closest && 
          (syntheticEvent.target.closest('.block-inner-dz') || syntheticEvent.target.closest('.block-row'))) return;
      dragData = touchDragInfo;
      handleDrop(blocks, blocks.length);
    };
