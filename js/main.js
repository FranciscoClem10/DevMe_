...
consoleTabs.forEach(t => {
  t.addEventListener('click', () => {
    ...
    if (tabId !== 'console') {
      ...
      // Si estamos en la pestaña de consola y hay una ejecución activa esperando input, restaurar el input
      if (isRunning && inputLineEl.dataset.waiting === 'true') {
  ...
  function waitForInput(prompt) {
    ...
    if (consoleTab) consoleTab.click();
      ...
      inputLine.dataset.waiting = 'true';
      ...
      const cleanup = () => {
        inputLine.dataset.waiting = 'false';
  ...
  async function renderMermaid(mermaidCode) {
    ...
    try {
      ...
      console.warn('Diagrama omitido:', e.message || e);
      container.innerHTML = '<div style="padding:20px;color:#9c8749;text-align:center;">Diagrama no disponible para este código.</div>';
    }
  }
  ...
  async function runProgram() {
    ...
    // Detectar si estamos en un layout de juego
    const isGameMode = (currentLayout === 'game' || currentLayout === 'split-code' || currentLayout === 'split-blocks');
    ...
    // Si estamos en modo juego, preparar el estado del juego
    if (isGameMode && typeof window.prepareGameState === 'function') {
      await window.prepareGameState(abortController.signal);
      // Actualizar UI del juego
      const gameStatusChip = document.getElementById('game-status-chip');
      if (gameStatusChip) {
        gameStatusChip.className = 'game-chip game-chip-running';
        gameStatusChip.textContent = 'Ejecutando...';
      }
      const btnPause = document.getElementById('btn-pause');
    ...
    try {
      ...
      if (err.message === 'Ejecucion cancelada' || err.message === 'Ejecución cancelada') writelnOut('--- Ejecucion cancelada ---', 'info');
      else if (err.message === '__WIN__') {
        // Victoria durante la ejecución (objetivos completados mid-programa)
        // checkGameGoals en el finally se encargará de mostrar la pantalla de victoria
        writelnOut('--- Objetivos completados ---', 'info');
      }
      else {
        showError({ phase: err.phase || 'Ejecucion', message: err.message, line: err.line || 0 });
        // Si estamos en modo juego, mostrar overlay con el error
        if (isGameMode && typeof window.showGameOverlay === 'function') {
          const msg = err.message || 'Error de ejecucion';
          window.showGameOverlay(msg, true, { line: err.line || 0 });
        } else {
          switchToConsoleTab('errors');
        }
      }
      ...
      // Si estamos en modo juego, verificar objetivos y limpiar
      if (isGameMode) {
        if (typeof window.checkGameGoals === 'function') {
          window.checkGameGoals();
        }
        if (typeof window.cleanupGameState === 'function') {
          window.cleanupGameState();
        }
  ...
  btnRun.addEventListener('click', () => {
      // Siempre usar el intérprete estándar unificado
      // El diagrama se genera dentro de runProgram() con manejo de errores
      runProgram();
  });
