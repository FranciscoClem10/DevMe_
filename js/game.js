...
function stopGameProgram(){
  // En el flujo unificado, el abortController está en main.js
  // pero el signal se pasa al gameState. Solo necesitamos resumir si está pausado.
...
function resetGame(){
  // Abortar ejecución si está corriendo (señal abort)
  if(window.__gameState && window.__gameState.signal && !window.__gameState.signal.aborted) {
    // No podemos abortar directamente (el AbortController está en main.js)
    // Pero podemos resumir si está pausado
  }
  if(window.__gameState && window.__gameState.paused) window.__gameState.resume();
  stopEnemyPatrol();
...
// ============================================================
// prepareGameState() — Configura window.__gameState para que
// el intérprete estándar pueda ejecutar los BUILTINS del juego.
// ============================================================
async function prepareGameState(signal){
  stopEnemyPatrol();
  resetEnemiesToOrigin();
  await new Promise(r => setTimeout(r, 350));
  ...
  if (typeof recalcLaserBeams === 'function') recalcLaserBeams(world);
  ...
  const state = {
    ...
    signal: signal || null,
    instrCount: 0,
    stepDelay: () => (850 - parseInt(gameSpeed.value, 10)),
    log: gameLog,
    stopped: false,
    paused: false,
    pauseResolve: null
...
// Verificar objetivos del juego después de la ejecución del intérprete estándar
function checkGameGoals(){
  if(!world) return;
  renderer.render();
  updateGoalsUI();
  if(checkGoalsSilent()){
    const n = (window.__gameState && window.__gameState.instrCount) || (parseInt($('game-instr-count').textContent,10)||0);
    onGameWin(n);
  } else {
    setGameStatus('idle','Fin del programa');
    const instrCount = (window.__gameState && window.__gameState.instrCount) || 0;
    gameLog(`Programa terminado (${instrCount} instrucciones). Objetivo no alcanzado.`, 'info');
  }
}
...
window.prepareGameState = prepareGameState;
window.cleanupGameState = cleanupGameState;
window.checkGameGoals = checkGameGoals;
