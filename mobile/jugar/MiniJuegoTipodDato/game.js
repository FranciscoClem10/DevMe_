...
class Game {
    ...
    showFeedback(correct, dataType, shotType) {
        ...
        if (correct) {
            ...
            msg = { text: '✖ No es ' + WEAPON_CONFIG[shotType].name, color: '#dc2626', timer: 0.8, y: 0 };
        }
    ...
    drawHUD() {
        ...
        ctx.fillStyle = 'rgba(252, 251, 248, 0.92)';
        ...
        for (let i = 0; i < 4; i++) {
            ...
            ctx.textAlign = 'center';
            ctx.fillStyle = isSelected ? wConfig.color : '#9c8749';
    ...
    drawOverlays() {
        ...
        if (this.gameState === 'MENU') {
            ...
            this.drawOverlay('PAUSA', 'Toca para continuar', '');
        } else if (this.gameState === 'GAMEOVER') {
            this.drawOverlay('GAME OVER', `Puntaje Final: ${this.score}`, 'Toca para volver al menú');
        } else if (this.gameState === 'LEVEL_TRANSITION') {
            this.drawLevelTransition();
        }

        if (this.escapeAlertTimer > 0 && this.gameState === 'PLAYING') {
            ctx.fillStyle = `rgba(220, 38, 38, ${Math.min(0.85, this.escapeAlertTimer)})`;
    ...
    drawMenuOverlay() {
        ...
        ctx.fillStyle = '#1c180d';
        ctx.font = '12px "Courier New"';
        ctx.fillText('Joystick: Mover nave', this.width / 2, instY);
        ctx.fillText('Toca para disparar', this.width / 2, instY + 20);
        ctx.fillText('Botones: Cambiar arma', this.width / 2, instY + 40);
        ctx.fillText('Botón pausa', this.width / 2, instY + 60);

        ctx.fillStyle = '#b8860b';
        ctx.font = 'bold 18px "Courier New"';
        let blink = Math.sin(Date.now() / 300) > 0;
        if (blink) {
            ctx.fillText('Toca para iniciar', this.width / 2, this.height / 2 + 200);
        }
    ...
    drawLevelTransition() {
        ...
        ctx.fillText(`Continuando en ${remaining}s... (Toca para saltar)`, this.width / 2, this.height / 2 + 80);
    }
    ...
    updateWeaponButtons() {
        ...
        btns.forEach((btn, i) => {
            if (i === this.player.selectedWeapon) {
